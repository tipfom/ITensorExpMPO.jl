using ITensorMPS:
  QNMatElem,
  which_op,
  MatElem,
  posInLink!,
  coefficient,
  remove_dups!,
  truncate!,
  Block,
  computeSiteProd,
  terms,
  site,
  params,
  toMatrix
using ITensors.NDTensors: using_auto_fermion
using ITensors: QNIndex

function qn_finitestatemachine(
  ElT::Type{<:Number}, os::OpSum{C}, sites; mindim=1, maxdim=typemax(Int), cutoff=1e-15
) where {C}
  N = length(sites)

  # Specifying the element type with `Dict{QN,Matrix{ElT}}[...]` improves type inference and therefore efficiency.
  # See https://github.com/ITensor/ITensors.jl/pull/1183.
  Vs = Dict{QN,Matrix{ElT}}[Dict{QN,Matrix{ElT}}() for n in 1:(N + 1)]
  sparse_MPO = [QNMatElem{Scaled{C,Prod{Op}}}[] for n in 1:N]

  function crosses_bond(t::Scaled{C,Prod{Op}}, n::Int)
    return (only(site(t[1])) <= n <= only(site(t[end])))
  end

  # A cache of the ITensor operators on a certain site
  # of a certain type
  op_cache = Dict{Pair{String,Int},ITensor}()
  function calcQN(term::Vector{Op})
    q = QN()
    for st in term
      op_tensor = get(op_cache, which_op(st) => only(site(st)), nothing)
      if op_tensor === nothing
        op_tensor = op(sites[only(site(st))], which_op(st); params(st)...)
        op_cache[which_op(st) => only(site(st))] = op_tensor
      end
      q -= flux(op_tensor)
    end
    return q
  end

  Hflux = -calcQN(terms(first(terms(os))))
  @show Hflux

  rightmap = Dict{Pair{Vector{Op},QN},Int}()
  next_rightmap = Dict{Pair{Vector{Op},QN},Int}()

  for n in 1:N
    h_sparse = Dict{QN,Vector{MatElem{ElT}}}()

    leftmap = Dict{Pair{Vector{Op},QN},Int}()
    for term in os
      crosses_bond(term, n) || continue

      left = filter(t -> (only(site(t)) < n), terms(term))
      onsite = filter(t -> (only(site(t)) == n), terms(term))
      right = filter(t -> (only(site(t)) > n), terms(term))

      lqn = calcQN(left)
      sqn = calcQN(onsite)

      bond_row = -1
      bond_col = -1
      if !isempty(left)
        bond_row = posInLink!(leftmap, left => lqn)
        bond_col = posInLink!(rightmap, vcat(onsite, right) => lqn)
        bond_coef = convert(ElT, coefficient(term))
        q_h_sparse = get!(h_sparse, lqn, MatElem{ElT}[])
        push!(q_h_sparse, MatElem(bond_row, bond_col, bond_coef))
      end

      rqn = sqn + lqn
      A_row = bond_col
      A_col = posInLink!(next_rightmap, right => rqn)
      site_coef = one(C)
      if A_row == -1
        site_coef = coefficient(term)
      end
      if isempty(onsite)
        if !using_auto_fermion() && isfermionic(right, sites)
          push!(onsite, Op("F", n))
        else
          push!(onsite, Op("Id", n))
        end
      end
      el = QNMatElem(lqn, rqn, A_row, A_col, site_coef * Prod(onsite))
      push!(sparse_MPO[n], el)
    end
    remove_dups!(sparse_MPO[n])

    if n > 1 && !isempty(h_sparse)
      for (q, mat) in h_sparse
        h = toMatrix(mat)
        U, S, V = svd(h)
        P = S .^ 2
        truncate!(P; maxdim, cutoff, mindim)
        tdim = length(P)
        Vs[n][q] = Matrix{ElT}(V[:, 1:tdim])
      end
    end

    rightmap = next_rightmap
    next_rightmap = Dict{Pair{Vector{Op},QN},Int}()
  end

  return Hflux, sparse_MPO, Vs
end

function eulermpo_qn(
  dt, os::OpSum{C}, sites, alg::Algorithm; mindim=1, maxdim=typemax(Int), cutoff=1e-15
)::MPO where {C}
  ElT = promote_type(eltype(dt), C)
  N = length(sites)

  Hflux, sparse_MPO, Vs = qn_finitestatemachine(ElT, os, sites; mindim, maxdim, cutoff)

  @assert Hflux == zero(Hflux) "total flux must be zero"
  #
  # Make MPO link indices
  #
  llinks = Vector{QNIndex}(undef, N + 1)
  xlinks = Vector{QNIndex}(undef, N + 1)
  # Set dir=In for fermionic ordering, avoid arrow sign
  # <fermions>:
  linkdir = using_auto_fermion() ? ITensors.In : ITensors.Out
  llinks[1] = Index([Hflux => 1]; tags="Link,l=0", dir=linkdir)
  xlinks[1] = Index([Hflux => 0]; tags="xLink,l=0", dir=linkdir)
  # llinks[1] = Index([Hflux => 1]; tags="Link,l=0", dir=linkdir)
  for n in 1:N
    qi = Vector{Pair{QN,Int}}()
    # push!(qi, QN() => 1)
    for (q, Vq) in Vs[n + 1]
      cols = size(Vq, 2)
      if using_auto_fermion() # <fermions>
        push!(qi, (-q) => cols)
      else
        push!(qi, q => cols)
      end
    end
    xlinks[n + 1] = Index(qi..., Hflux => 0; tags="xLink,l=$n", dir=linkdir)
    push!(qi, Hflux => 1)
    llinks[n + 1] = Index(qi...; tags="Link,l=$n", dir=linkdir)
  end

  U = MPO(N)

  # Find location where block of Index i
  # matches QN q, but *not* 1 or dim(i)
  # which are special ending/starting states
  function qnblock(i::Index, q::QN)
    for b in 1:nblocks(i)
      flux(i, Block(b)) == q && return b
    end
    return error("Could not find block of QNIndex with matching QN")
  end
  function qnblock2(i::Index, q::QN)
    for b in 2:(nblocks(i) - 1)
      flux(i, Block(b)) == q && return b
    end
    return error("Could not find block of QNIndex with matching QN")
  end

  qnblockdim(i::Index, q::QN) = blockdim(i, qnblock(i, q))

  for n in 1:N
    @show n
    ll = llinks[n]
    rl = llinks[n + 1]

    begin_block = Dict{Tuple{QN,Vector{Op}},Matrix{ElT}}()
    cont_block = Dict{Tuple{QN,Vector{Op}},Matrix{ElT}}()
    end_block = Dict{Tuple{QN,Vector{Op}},Matrix{ElT}}()
    onsite_block = Dict{Tuple{QN,Vector{Op}},Matrix{ElT}}()

    for el in sparse_MPO[n]
      t = el.val
      (abs(coefficient(t)) > eps()) || continue
      A_row = el.row
      A_col = el.col
      ct = convert(ElT, coefficient(t))

      ldim = (A_row == -1) ? 1 : qnblockdim(ll, el.rowqn)
      rdim = (A_col == -1) ? 1 : qnblockdim(rl, el.colqn)
      zero_mat() = zeros(ElT, ldim, rdim)

      if A_row == -1 && A_col == -1
        # Onsite term
        M = get!(onsite_block, (el.rowqn, terms(t)), zeros(ElT, 1, 1))
        M[1, 1] += ct
      elseif A_row == -1
        # Operator beginning a term on site n
        M = get!(begin_block, (el.rowqn, terms(t)), zero_mat())
        VR = Vs[n + 1][el.colqn]
        for c in 1:size(VR, 2)
          M[1, c] += ct * VR[A_col, c]
        end
      elseif A_col == -1
        # Operator ending a term on site n
        M = get!(end_block, (el.rowqn, terms(t)), zero_mat())
        VL = Vs[n][el.rowqn]
        for r in 1:size(VL, 2)
          M[r, 1] += ct * conj(VL[A_row, r])
        end
      else
        # Operator continuing a term on site n
        M = get!(cont_block, (el.rowqn, terms(t)), zero_mat())
        VL = Vs[n][el.rowqn]
        VR = Vs[n + 1][el.colqn]
        for r in 1:size(VL, 2), c in 1:size(VR, 2)
          M[r, c] += ct * conj(VL[A_row, r]) * VR[A_col, c]
        end
      end
    end

    U[n] = ITensor()

    function get_rq(q_op)
      return q_op[1]
    end

    function get_Op(q_op)
      op_prod = q_op[2]
      return Op = computeSiteProd(sites, Prod(op_prod))
    end

    function get_cq(q_op)
      Op = get_Op(q_op)

      rq = q_op[1]
      sq = flux(Op)
      cq = rq
      if !isnothing(sq)
        # By convention, if `Op` has no blocks it has a flux
        # of `nothing`, catch this case
        cq -= sq

        if using_auto_fermion()
          # <fermions>:
          # MPO is defined with Index order
          # of (rl,s[n]',s[n],cl) where rl = row link, cl = col link
          # so compute sign that would result by permuting cl from
          # second position to last position:
          if fparity(sq) == 1 && fparity(cq) == 1
            Op .*= -1
          end
        end
      end
      return cq
    end

    x = xlinks[n]
    y = xlinks[n + 1]

    @show s = sites[n]
    _A = ITensor(Float64, dag(x), y, dag(s), s')
    _B = ITensor(Float64, dag(x), dag(s), s')
    _C = ITensor(Float64, y, dag(s), s')
    _D = ITensor(Float64, dag(s), s')
    @show _D

    for (q_op, M) in onsite_block
      @show "D", q_op, M, get_Op(q_op)
      @assert length(M) == 1
      _D += M[1, 1] * get_Op(q_op)
    end
    for (q_op, M) in begin_block
      @show "C", q_op, M
      b = Block(qnblock(y, get_cq(q_op)))
      T = ITensors.NDTensors.BlockSparseTensor(ElT, [b], y)
      T[b] .= transpose(M)

      _C += itensor(T) * get_Op(q_op)
    end
    for (q_op, M) in end_block
      @show "B", q_op, M
      b = Block(qnblock(x, get_rq(q_op)))
      T = ITensors.NDTensors.BlockSparseTensor(ElT, [b], dag(x))
      T[b] .= M

      _B += itensor(T) * get_Op(q_op)
    end
    for (q_op, M) in cont_block
      @show "A", q_op, M
      b = Block(qnblock(x, get_rq(q_op)), qnblock(y, get_cq(q_op)))
      @show b
      T = ITensors.NDTensors.BlockSparseTensor(ElT, [b], (dag(x), y))
      T[b] .= M

      _A += itensor(T) * get_Op(q_op)
    end

    W = makeW(alg, ElT, dt, dense(_A), dense(_B), dense(_C), dense(_D))
    @show size(W), ll, rl, s
    @show ITensorMPS.itensor(W, dag(rl), ll, dag(s), s'; checkflux=false)
    U[n] = ITensorMPS.itensor(W, dag(ll), rl, dag(s), s'; checkflux=false)
  end

  L = ITensor(llinks[1])
  L[llinks[1] => end] = 1.0
  U[1] *= L

  R = ITensor(dag(llinks[N + 1]))
  R[dag(llinks[N + 1]) => 1] = 1.0
  U[N] *= R

  return U
end